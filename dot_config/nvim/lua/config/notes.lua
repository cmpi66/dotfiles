...
local ROOTS = {
  ...
  vim.fn.expand("~/.local/share/notes/pentest-hub"),
  vim.fn.expand("~/.local/share/notes/blueteam-hub"),
}
...
-- Returns the notebook root for the current buffer, or falls back to ROOT
local function current_root()
  local buf = vim.fn.expand("%:p")
  for _, r in ipairs(ROOTS) do
    if buf:sub(1, #r) == r then
      return r
    end
  end
  return ROOT
...
local function rel_to_root(abs, root)
  root = root or ROOT
  local rootp = norm_dir(root)
...
-- Parses YAML front-matter tags from a file
...
function M.new_note(dir)
  ...
  local rel_dir = rel_to_root(dir, root)
  ...
  vim.notify("Created: " .. rel_to_root(path, root), vim.log.levels.INFO)
end
...
function M.search_by_tag()
  local root = current_root()
  local cmd = ("find %s -type f -name '*.md' 2>/dev/null"):format(vim.fn.shellescape(root))
  local h = io.popen(cmd)
  if not h then
    return
  end
  ...
  local tag_map = {}
  for filepath in h:lines() do
    for _, tag in ipairs(get_tags(filepath)) do
  ...
  for tag in pairs(tag_map) do
    table.insert(tags, string.format("%-20s  (%d)", tag, #tag_map[tag]))
  end
  table.sort(tags)

  if #tags == 0 then
    vim.notify("No tagged notes found under " .. root, vim.log.levels.WARN)
    return
  end

  vim.ui.select(tags, { prompt = "Select tag:" }, function(chosen)
    if not chosen then
      return
    end
